...
public class DetailPage extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//generate json format for user session 
		 HashMap UserSession = ratingController.ReturnJsonKey("UserSession",session.getAttribute("UserSession"));
		 HashMap GlobalRating = ratingController.ReturnJsonKey("GlobalRating",(HashMap<String,HashMap<Integer,Integer>>) getServletContext().getAttribute("GlobalRating"));
		 HashMap GlobalProCount = ratingController.ReturnJsonKey("GlobalProCount",(HashMap<String,HashMap<Integer,Integer>>) getServletContext().getAttribute("GlobalProCount"));
		 HashMap GlobalProAvge = ratingController.ReturnJsonKey("GlobalProAvge",(HashMap<String,HashMap<Integer,Integer>>) getServletContext().getAttribute("GlobalProAvge"));
		 HashMap JsonHashMap = new HashMap();
		 JsonHashMap.putAll(UserSession);
		 JsonHashMap.putAll(GlobalRating);
		 JsonHashMap.putAll(GlobalProCount);
		 JsonHashMap.putAll(GlobalProAvge);

		 response.setContentType("text/html");
	     PrintWriter printWriter  = response.getWriter();
	     printWriter.println(JSONValue.toJSONString(JsonHashMap));
